...
func (p *Post) SavePost(db *gorm.DB) (*Post, error) {
	var err error = db.Debug().Model(&Post{}).Create(&p).Error
	if err != nil {
		return &Post{}, err
	}
	if p.ID != 0 {
...
func (p *Post) FindPostByID(db *gorm.DB, pid uint64) (*Post, error) {
	var err error = db.Debug().Model(&Post{}).Where("id = ?", pid).Take(&p).Error
	if err != nil {
		return &Post{}, err
	}
	if p.ID != 0 {
...
func (p *Post) UpdateAPost(db *gorm.DB) (*Post, error) {
	var err error = db.Debug().Model(&Post{}).Where("id = ?", p.ID).Updates(Post{Title: p.Title, Content: p.Content, UpdatedAt: time.Now()}).Error
	if err != nil {
		return &Post{}, err
	}
	if p.ID != 0 {
