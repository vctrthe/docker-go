...
func TestSignIn(t *testing.T) {
	...
	samples := []struct {
		...
		{
			email:        user.Email,
			password:     "password", // Note the password has to be this, not the hashed one from the DB
			errorMessage: "",
		},
		{
			email:        user.Email,
...
func TestLogin(t *testing.T) {
	...
	samples := []struct {
		...
		{
			inputJSON:    `{"email": "victor@gmail.com", "password": "password"}`,
			statusCode:   200,
			errorMessage: "",
		},
		{
			inputJSON:    `{"email": "victor@gmail.com", "password": "wrong password"}`,
			statusCode:   422,
